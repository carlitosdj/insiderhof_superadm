...
import { availableModule } from "../../../../store/ducks/davailablemodule/types";
...
const ManageAvailable = ({ handleClose, child }: handleCloseProps) => {
  const [inputList, setInputList] = useState([
    { launchId: "", availableDate: "", moduleId: child.id, deadline: "" },
  ]);
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    let newComponent: Module = {};

    newComponent.id = child.id;
    newComponent.availableModule = [];

    inputList.map((it) => {
      if (it.launchId && it.moduleId)
        newComponent.availableModule?.push({
          launchId: parseInt(it.launchId),
    ...
    dispatch(updateModuleRequest(newComponent));
    handleClose();
  };

  const setConfigItems = (list: availableModule[]) => {
  ...
  useEffect(() => {
    ...
    setConfigItems(child.availableModule!);
