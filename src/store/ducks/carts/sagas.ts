...
export function* loadCarts(payload: ReturnType<typeof loadCartsRequest>) {
  try {
    ...
    if (
      ...
      console.log(
        "entrou",
        `cart/all/${payload.payload.startDate}/${payload.payload.endDate}/${payload.payload.launchId}`
      );
      response = yield call(
        api.get,
        `cart/all/${payload.payload.startDate}/${payload.payload.endDate}/${payload.payload.launchId}`
...
export function* updateCart(payload: ReturnType<typeof updateCartRequest>) {
  try {
    ...
    const response: Cart = yield call(
      api.patch,
      "cart/" + payload.payload.id,
      payload.payload
    );
...
//SelectedUser
export function* selectCartsAdd(
  payload: ReturnType<typeof selectCartsAddRequest>
) {
  try {
...
//SelectedUser
export function* selectCartsRemove(
  payload: ReturnType<typeof selectCartsRemoveRequest>
) {
  try {
    //const response: User = yield call(api.delete, 'users/' + payload.payload)
